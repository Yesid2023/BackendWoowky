          ...
          <div class="d-grid">
            <div class="d-flex flex-column">
              <div class="mb-4">
                ...
                <!-- <Multiselect v-model="assign_ids" placeholder="Select pet"  :value="assign_ids" :canClear="false" v-bind="singleSelectOption" :options="pets.options" @select="selectEmployee" id="employees_ids">
                  <template v-slot:multiplelabel="{ values }">
                    <div class="multiselect-multiple-label">Select pet</div>
                  </template>
                </Multiselect> -->
              </div>
            </div>
            <div class="list-group list-group-flush">
              <div v-for="(item, index) in selectedpets" :key="item" class="list-group-item d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center flex-grow-1 gap-2 my-2">
                  ...
                  <!-- <div class="flex-grow-1 mt-2"> {{$t('pet.breed')}} {{ item.breed ? item.breed.name : '-' }}</div> -->
                ...
                <button type="button" class="fs-4 text-primary border-0 bg-transparent" data-bs-toggle="offcanvas" data-bs-target="#pet-form-offcanvas"    @click="changeId(item.id)"  aria-controls="form-offcanvas" ><i class="icon-Edit"></i></button>
                <button type="button" class="fs-4 text-danger border-0 bg-transparent" @click="removePet(item.id, 'Are you certain you want to delete it?')"  data-bs-toggle="tooltip"> <i class="icon-delete"></i></button>
