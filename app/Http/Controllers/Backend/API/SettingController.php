...
class SettingController extends Controller
{
    public function appConfiguraton(Request $request)
    {
        ...
        foreach ($settings as $name => $value) {
            if (in_array($name, $specificNames)) {
                if (strpos($name, 'onesignal_') === 0 && $request->is_authenticated == 1) {
                    ...
                    $response[$nestedKey][$nestedName] = $value;
                } elseif (strpos($name, 'customer_app_') === 0 && $request->is_authenticated == 1) {
                    ...
                    $response[$nestedKey][$nestedName] = $value;
                } elseif (strpos($name, 'employee_app_') === 0  && $request->is_authenticated == 1) {
        ...
        if(isset($settings['is_zoom']) && isset($settings['account_id']) && isset($settings['client_id']) && isset($settings['client_secret'])){
            $zoomData = [
